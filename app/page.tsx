...
export default function Home() {
  const [initialRoom] = useState(inviteRoomFromUrl);
  const [room, setRoom] = useState(initialRoom);
  const [isGuest] = useState(Boolean(initialRoom));
  const [status, setStatus] = useState<'idle' | 'waiting' | 'connecting' | 'connected' | 'error'>('idle');
  const [message, setMessage] = useState(initialRoom ? '초대 링크가 확인되었습니다. 자동 연결을 시작합니다.' : '공유할 항목을 준비한 뒤 새 방을 만드세요.');
  ...
  useEffect(() => () => {
    void session.current?.leave();
  }, []);

  const connectSession = useCallback((id: string) => {
    void session.current?.leave();
    setStatus('connecting');
    setMessage('분산 신호망에서 상대방을 찾고 안전한 직접 연결을 준비하고 있습니다.');
    const roomSession = joinTrysteroRoom(
      { appId: 'somoim-webrtc-v1', password: id },
      id,
      {
        onJoinError: () => {
          setStatus('error');
          setMessage('직접 연결을 만들지 못했습니다. 네트워크를 확인하고 다시 시도해주세요.');
        },
      },
    );
    ...
    roomSession.onPeerStream = (stream) => setRemoteStream(stream);
    roomSession.onPeerJoin = (peerId) => {
      setStatus('connected');
      setMessage('상대방과 직접 연결되었습니다. 이제 화면을 보거나 파일을 전송할 수 있습니다.');
      if (localStreamRef.current) void roomSession.addStream(localStreamRef.current, { target: peerId, metadata: { kind: 'screen' } });
    };
    roomSession.onPeerLeave = () => {
      setRemoteStream(null);
      setStatus('waiting');
      setMessage('상대방 연결이 종료되었습니다. 같은 링크로 다시 접속할 수 있습니다.');
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (/^[a-f0-9]{32}$/.test(initialRoom)) connectSession(initialRoom);
      else {
        setStatus('error');
  ...
  const prepareMedia = async () => {
    try {
      ...
      display.getVideoTracks()[0]?.addEventListener('ended', () => {
        session.current?.removeStream(combined);
        localStreamRef.current = null;
        setLocalStream(null);
      });
      localStreamRef.current = combined;
      setLocalStream(combined);
      void session.current?.addStream(combined, { metadata: { kind: 'screen' } });
      setMessage(session.current ? '화면, 시스템 소리, 마이크 공유를 시작했습니다.' : '화면, 시스템 소리, 마이크가 준비되었습니다. 이제 새 방을 만드세요.');
    } catch {
  ...
  const createRoom = () => {
    ...
    setRoom(id);
    setStatus('waiting');
    window.history.replaceState({}, '', `${window.location.pathname}?room=${id}`);
    connectSession(id);
  ...
  const stopSharing = () => {
    if (localStream) session.current?.removeStream(localStream);
  ...
  const sendFiles = async () => {
    const send = sendTransfer.current;
    if (!send || status !== 'connected' || files.length === 0) {
      ...
      <header className="border-b border-white/8 bg-[#07111f]/92 px-5 py-4 text-white backdrop-blur-xl sm:px-8">
        <div className="mx-auto flex max-w-[1440px] items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-[15px] font-bold tracking-tight">somoim</p><p className="text-[11px] text-slate-400">WebRTC peer-to-peer</p></div>
          </div>
      ...
      <section className="border-b border-border bg-[#0a1628] px-5 py-8 text-white sm:px-8">
        <div className="mx-auto grid max-w-[1440px] gap-6 lg:grid-cols-[1fr_auto] lg:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">전송 데이터는 두 기기 사이를 직접 이동합니다. 방 코드는 분산 신호 발견과 연결 암호화에만 사용됩니다.</p>
          </div>
          <div className="flex flex-wrap gap-2 text-xs text-slate-300"><span className="feature-pill"><ShieldCheck size={14} /> 파일 서버 저장 없음</span><span className="feature-pill"><FileArchive size={14} /> 대용량 분할 전송</span></div>
        </div>
      </section>
        ...
        <aside className="space-y-5">
          <section className="panel sticky top-5">
            ...
            <div className="space-y-4 p-5">
              ...
              {isGuest ? status === 'error' && <button className="button-primary w-full justify-center py-3" onClick={joinRoom}><Link2 size={17} /> 다시 연결</button> : !room ? <button className="button-primary w-full justify-center py-3" onClick={createRoom}><Radio size={17} /> 새 공유 방 만들기</button> : <button className="button-secondary w-full justify-center" onClick={copyInvite}>{copied ? <Check size={16} /> : <Clipboard size={16} />} {copied ? '복사됨' : '초대 링크 복사'}</button>}
